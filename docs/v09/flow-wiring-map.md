...
| W18    | S1             | UI/Job                   | HTTP Endpoint        | POST /api/loop/issues/{issueId}/run-next-step | control | issueId, mode?, actor, requestId | runId, stepExecuted, loopStatus | loop run record (loop_runs) + timeline event | issueId + mode + actor (idempotency) | 401/404/409/400/500 | E9.1-proof: run-e91-s1-001 | [control-center/app/api/loop/issues/[issueId]/run-next-step/route.ts](control-center/app/api/loop/issues/[issueId]/run-next-step/route.ts) `POST`; S1 executor: Pick/Link GitHub issue (E9.1-CTRL-5) |
| W19    | S2             | UI/Job                   | HTTP Endpoint        | POST /api/loop/issues/{issueId}/run-next-step | control | issueId, mode?, actor, requestId | runId, stepExecuted, loopStatus | loop run record (loop_runs) + timeline event | issueId + mode + actor (idempotency) | 401/404/409/400/500 | E9.1-proof: run-e91-s2-001, run-e91-s2-002 | [control-center/app/api/loop/issues/[issueId]/run-next-step/route.ts](control-center/app/api/loop/issues/[issueId]/run-next-step/route.ts) `POST`; S2 executor: Spec Ready gate (E9.1-CTRL-6) |
| W20    | S3             | UI/Job                   | HTTP Endpoint        | POST /api/loop/issues/{issueId}/run-next-step | control | issueId, mode?, actor, requestId | runId, stepExecuted, loopStatus | loop run record (loop_runs) + timeline event | issueId + mode + actor (idempotency) | 401/404/409/400/500 | E9.1-proof: run-e91-s3-001 | [control-center/app/api/loop/issues/[issueId]/run-next-step/route.ts](control-center/app/api/loop/issues/[issueId]/run-next-step/route.ts) `POST`; S3 executor: Implement Prep - create/attach PR (E9.1-CTRL-7) |
| W21    | S1-S9          | UI/Job                   | HTTP Endpoint        | GET /api/loop/issues/{issueId}/events | control | issueId, limit?, offset? | events[], total, schemaVersion | none (read-only; timeline events query) | issueId + pagination | 401/404/400/500 | E9.1-proof: Used for all event queries | [control-center/app/api/loop/issues/[issueId]/events/route.ts](control-center/app/api/loop/issues/[issueId]/events/route.ts) `GET`; Timeline events query (E9.1-CTRL-8) |
...
| E18 | POST | /api/loop/issues/{issueId}/run-next-step | control | Execute next step in loop for given AFU-9 issue (S1/S2/S3/...). | UI/Job | W18, W19, W20 | Y | [control-center/app/api/loop/issues/[issueId]/run-next-step/route.ts](control-center/app/api/loop/issues/[issueId]/run-next-step/route.ts) `POST`; E9.1-CTRL-1 Loop API |
| E19 | GET | /api/loop/issues/{issueId}/events | control | Query timeline events for a loop execution by issue ID. | UI/Job | W21 | N | [control-center/app/api/loop/issues/[issueId]/events/route.ts](control-center/app/api/loop/issues/[issueId]/events/route.ts) `GET`; E9.1-CTRL-8 Timeline Events |
