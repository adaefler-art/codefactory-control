import { Pool } from 'pg';
import { RunSpec, RunResult, StepResult } from '../contracts/schemas';

/**
 * RunsDAO - Database Access Object for Runs Ledger (I632)
 * 
...
export class RunsDAO {
   ...
   */
  async createRun(runId: string, spec: RunSpec, issueId?: string, playbookId?: string, parentRunId?: string): Promise<void> {
    ...
    try {
      ...
      await client.query(
        ...
        [runId, issueId || null, spec.title, playbookId || null, parentRunId || null, 'QUEUED', JSON.stringify(spec)]
      );
   ...
   * Update run status
   */
  async updateRunStatus(runId: string, status: string, startedAt?: Date, finishedAt?: Date): Promise<void> {
  ...
  async updateRunResult(runId: string, resultJson: any): Promise<void> {
    await this.pool.query(
      'UPDATE runs SET result_json = $2 WHERE id = $1',
      [runId, JSON.stringify(resultJson)]
    );
  }
   ...
   */
  async listRunsByIssue(issueId: string, limit: number = 50, offset: number = 0): Promise<any[]> {
    const result = await this.pool.query(
      `SELECT * FROM runs WHERE issue_id = $1 ORDER BY created_at DESC LIMIT $2 OFFSET $3`,
   ...
   * Map database run status to RunResult status
   */
  private mapRunStatus(dbStatus: string): 'created' | 'running' | 'success' | 'failed' | 'timeout' | 'cancelled' {
    switch (dbStatus) {
      case 'QUEUED':
        return 'created';
      case 'RUNNING':
   ...
   */
  private mapStepStatus(dbStatus: string): 'pending' | 'running' | 'success' | 'failed' | 'timeout' | 'skipped' {
    switch (dbStatus) {
      case 'QUEUED':
        return 'pending';
      case 'RUNNING':
